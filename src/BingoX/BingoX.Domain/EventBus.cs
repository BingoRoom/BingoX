...
namespace BingoX.Domain
{
    public class EventBus
    {
        ...
        private void MapEventToHandler()
        {
            ...
            {
                if (typeof(IDomainEventHandler).IsAssignableFrom(type))//判断当前类型是否实现了IEventHandler接口
                {
                    Type handlerInterface = type.GetInterface("IDomainEventHandler`1");//获取该类实现的泛型接口
