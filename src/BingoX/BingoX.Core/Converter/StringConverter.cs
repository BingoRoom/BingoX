...
namespace BingoX.Converter
{
    public static class StringConverter
    {
        ...
        /// <param name="ch"></param>        
        /// <returns></returns>        
        public static String GetPinYinOfFirstChar(Char ch)
        {
            var rs = GetPinYin(ch);
        ...
        public static String GetPinYinOfFirstChar(String str)
        {
            ...
            {
                sb.Append(GetPinYinOfFirstChar(t));
            }
        ...
        {
            // 拉丁字符            
            if (ch <= '\x00FF') return ch.ToString(CultureInfo.InvariantCulture);

            // 标点符号、分隔符            
            if (Char.IsPunctuation(ch) || Char.IsSeparator(ch)) return ch.ToString(CultureInfo.InvariantCulture);

            // 非中文字符            
            if (ch < '\x4E00' || ch > '\x9FA5') return ch.ToString(CultureInfo.InvariantCulture);

            var arr = Encoding.GetEncoding("gb2312").GetBytes(ch.ToString(CultureInfo.InvariantCulture));
            //Encoding.Default默认在中文环境里虽是GB2312，但在多变的环境可能是其它
            //var arr = Encoding.Default.GetBytes(ch.ToString()); 
            var chr = arr[0] * 256 + arr[1] - 65536;

            //***// 单字符--英文或半角字符  
            if (chr > 0 && chr < 160) return ch.ToString(CultureInfo.InvariantCulture);
            #region 中文字符处理
            ...
            {
                int pos = Array.IndexOf(OtherChinese, ch.ToString(CultureInfo.InvariantCulture));
        ...
        readonly static Regex RegexNumberToCn = new Regex(@"((?<=-|^)[^1-9]*)|((?'z'0)[0A-E]*((?=[1-9])|(?'-z'(?=[F-L\.]|$))))|((?'b'[F-L])(?'z'0)[0A-L]*((?=[1-9])|(?'-z'(?=[\.]|$))))");
        /// <summary>
        /// 金额转换
        /// </summary>
        ...
        {
            if (money > 999999999999.99) throw new Exception("数字太大，无法换算，请输入一万亿元以下的金额");
            if (Math.Abs(money) < Double.Epsilon) return string.Empty;

            string s = money.ToString("#L#E#D#C#K#E#D#C#J#E#D#C#I#E#D#C#H#E#D#C#G#E#D#C#F#E#D#C#.0B0A");
            string d = RegexNumberToCn.Replace(s, "${b}${z}");
            string cn = Regex.Replace(d, ".", m => "负元空零壹贰叁肆伍陆柒捌玖空空空空空空空分角拾佰仟万亿兆京垓秭穰"[m.Value[0] - '-'].ToString());
            return money < 0 ? "负" + cn : cn;
        }
